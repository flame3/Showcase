...
- (void)viewDidLoad
{
    ...
    [self.delagate shouldEnableCameraUI:NO];
    [self.delagate shouldEnableRecordUI:NO];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
	if ( context == CapturingStillImageContext ) {
		...
		dispatch_async( dispatch_get_main_queue(), ^{
            ...
            //self.delegate shouldEnableCameraUI:isSessionRunning
            [self.delagate shouldEnableCameraUI:isSessionRunning && ( [AVCaptureDevice devicesWithMediaType:AVMediaTypeVideo].count > 1 )];
            
            [self.delagate shouldEnableRecordUI:isSessionRunning];
...
- (void)changeCamera
{
	...
	dispatch_async( self.sessionQueue, ^{
		...
		dispatch_async( dispatch_get_main_queue(), ^{
            [self.delagate shouldEnableRecordUI:YES];
            [self.delagate shouldEnableCameraUI:YES];
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didStartRecordingToOutputFileAtURL:(NSURL *)fileURL fromConnections:(NSArray *)connections
{
	...
	dispatch_async( dispatch_get_main_queue(), ^{
        [self.delagate shouldEnableRecordUI:YES];
        [self.delagate recordingHasStarted];
	});
}
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error
{
	...
	if ( error ) {
        ...
        [self.delagate videoRecordingFailed];
	}
	if ( success ) {
        
        [self.delagate videoRecordingComplete:outputFileURL];
        /*
	...
	dispatch_async( dispatch_get_main_queue(), ^{
		// Only enable the ability to change camera if the device has more than one camera.
        [self.delagate shouldEnableRecordUI: [AVCaptureDevice devicesWithMediaType:AVMediaTypeVideo].count > 1 ];
        
        [self.delagate shouldEnableRecordUI:YES];
        [self.delagate canStartRecording];
	});
}
